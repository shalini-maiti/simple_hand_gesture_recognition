...
class skinDetector(object):
    def refine_mask(self, palmPoint):
        width = 30
        palmX = palmPoint[0]
        palmY = palmPoint[1]
        skinSample1 = self.HSV_image[palmY : palmY + width, palmX : palmX + width]
        skinSample2 = self.HSV_image[palmY + width : palmY+ (width * 2), palmX + width: palmX + (width * 2)]
                
        #Calculate thresholds based on new skin samples.
        offsetLowThreshold = 80
        offsetHighThreshold = 30
        
        hsvMeansSample1 = np.mean(skinSample1, axis=(0, 1))
        hsvMeansSample2 = np.mean(skinSample2, axis=(0, 1))
        
#        print("hsv means, sample 1:", hsvMeansSample1)
        ...
        #Segment skin based on thresholds. Cleanup with morphological ops. 
        skinMask = cv2.inRange(self.HSV_image, np.array((hLowThreshold, sLowThreshold, vLowThreshold)), np.array((hHighThreshold, sHighThreshold, vHighThreshold)))
        structuringElement = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
        skinMask  = cv2.morphologyEx(skinMask, cv2.MORPH_OPEN, structuringElement)
        skinMask = cv2.dilate(skinMask, structuringElement, iterations = 3)
        self.binary_mask_image = skinMask
...
#     empirical tests and visual evaluation
    def __color_segmentation(self):
        ...
        upper_HSV_values = np.array([255, 255, 255], dtype = "uint8")
